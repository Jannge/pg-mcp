...
async def _null_async_cm() -> AsyncIterator[None]:
    """No-op async context manager used when no rate limiter is configured."""
    yield
...
class QueryOrchestrator:
        ...
        try:
            ...
            if request.return_type == ReturnType.SQL:
                ...
                if self.metrics is not None:
                    self._record_query_request(database_name, "success")
            ...
            # Step 5: Execute SQL (rate-limited)
            logger.debug("Executing SQL", extra={"request_id": request_id})
            start_time = self._get_current_time_ms()
            async with (
                self.rate_limiter.for_queries() if self.rate_limiter is not None else _null_async_cm()
            ):
            ...
            self._record_query_request(request.database or "unknown", "error")
            return QueryResponse(
                success=False,
                generated_sql=None,
                validation=None,
    ...
    async def _generate_sql_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Generate SQL (rate-limited); capture real token usage when present
                gen_start_ms = self._get_current_time_ms()
                async with (
                    self.rate_limiter.for_llm() if self.rate_limiter is not None else _null_async_cm()
                ):
                    generated_sql, attempt_tokens = await self.sql_generator.generate(
                        question=question,
                        schema=schema,
                        previous_attempt=previous_sql,
                        error_feedback=error_feedback,
                    )
                gen_duration_s = (self._get_current_time_ms() - gen_start_ms) / 1000.0
                self._record_llm_metrics("generate_sql", gen_duration_s, attempt_tokens)
    ...
    async def _validate_results_safely(
        ...
        try:
            ...
            val_start_ms = self._get_current_time_ms()
            async with (
                self.rate_limiter.for_llm() if self.rate_limiter is not None else _null_async_cm()
            ):
            ...
            val_duration_s = (self._get_current_time_ms() - val_start_ms) / 1000.0
            self._record_llm_metrics("validate_result", val_duration_s, None)
    ...
    def _record_llm_metrics(self, operation: str, duration_s: float, tokens: int | None) -> None:
        """Record LLM call metrics when a MetricsCollector is configured."""
        if self.metrics is None:
            return
        self.metrics.increment_llm_call(operation)
        self.metrics.observe_llm_latency(operation, duration_s)
        if tokens is not None:
            self.metrics.increment_llm_tokens(operation, tokens)

    def _record_query_request(self, database: str, status: str) -> None:
        """Record a completed query request when a MetricsCollector is configured."""
        if self.metrics is None:
            return
