...
class SQLGenerator:
    ...
    Example:
        ...
        >>> sql, _ = await generator.generate(
    ...
    async def generate(
        ...
        Returns:
            tuple[str, int | None]: Generated SQL query (without trailing semicolon) and token usage.
        ...
        # Extract real token usage from the OpenAI response if available
        # (guarded so mocks without a `usage` attribute do not break)
